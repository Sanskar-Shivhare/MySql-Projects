...
SELECT 
    billing_country AS country, COUNT(*) count_of_invoices
FROM
    invoice
GROUP BY country
...
SELECT 
    billing_city AS city, SUM(total) total_amount
FROM
    invoice
GROUP BY city
...
with all_table as (select c.email,c.first_name,c.last_name,g.name from customer c inner join invoice i
on c.customer_id = i.customer_id inner join invoice_line il
on i.invoice_id = il.invoice_id inner join track t
on il.track_id = t.track_id inner join genre g
on t.genre_id = g.genre_id)

select email,first_name,last_name from all_table where name = "rock"
...
SELECT country,name FROM popular_genre WHERE RowNo <= 1;
